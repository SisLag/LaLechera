...
if (empty($_SESSION['usuarioEncargado']) && empty($_SESSION['claveEncargado'])){
    ...
    if ($_GET['act']=='insert') {
        if (isset($_POST['Guardar'])) {
            ...
            $query = mysqli_query($mysqli, "INSERT INTO inventario_medicamentos(codigo_inv,fecha,codigo,numero,tipo_transaccion) 
            ...
            if ($query) {
                ...
                $query1 = mysqli_query($mysqli, "UPDATE registrosmedicamentos SET stock = '$total_stock'
                                                              WHERE IdRegIcaMedicamento   = '$codigo'")
                                                or die('Error: '.mysqli_error($mysqli));

               
                if ($query1) {                       
                    
                    header("location: ../../main.php?module=inventario_medicamentos&alert=1");
                }
            }   
        }   
    }
