...
if (empty($_SESSION['usuarioEncargado']) && empty($_SESSION['claveEncargado'])){
    ...
    if ($_GET['act']=='insert') {
        if (isset($_POST['Guardar'])) {
            ...
            $query = mysqli_query($mysqli, "INSERT INTO inventario_productos(codigo_inv,fecha,codigo,numero,tipo_transaccion) 
            ...
            if ($query) {
                ...
                $query1 = mysqli_query($mysqli, "UPDATE registrosproductos SET stock = '$total_stock'
                                                              WHERE RegIcaProd   = '$codigo'")
                                                or die('Error: '.mysqli_error($mysqli));

               
                if ($query1) {                       
                    
                    header("location: ../../main.php?module=inventario_productos&alert=1");
                }
            }   
        }   
    }
