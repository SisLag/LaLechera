...
<script type="text/javascript">
  function tampil_obat(input){
    ...
    $.post("modules/inventario_alimentos/alimentos.php", {
      dataidobat: num,
    }, function(response) {      
      $('#stok').html(response)
...
if ($_GET['form'] == 'add') { ?> 
  ...
  <section class="content-header">
    <h1>
      <i class="fa fa-edit icon-title"></i> Datos entradas / salidas de alimentos
    </h1>
    <ol class="breadcrumb">
      <li><a href="?module=start"><i class="fa fa-home"></i> Inicio </a></li>
      <li><a href="?module=inventario_alimentos"> Entrada </a></li>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary">
          <!-- form start -->
          <form role="form" class="form-horizontal" action="modules/inventario_alimentos/proses.php?act=insert" method="POST" name="formObatMasuk">
            <div class="box-body">
              <?php 

              $query_id = mysqli_query($mysqli, "SELECT RIGHT(codigo_inv,7) as codigo FROM inventario_alimentos
                                                ORDER BY codigo_inv DESC LIMIT 1")
              ...
              $codigo_inv = "TM-$tahun-$buat_id";
              ?>

              <div class="form-group">
                <label class="col-sm-2 control-label">Código de Transacción </label>
                <div class="col-sm-5">
                  <input type="text" class="form-control" name="codigo_inv" value="<?php echo $codigo_inv; ?>" readonly required>
                </div>
              </div>
              ...
              <div class="form-group">  
                <label class="col-sm-2 control-label">Alimento</label>
                <div class="col-sm-5">
                  <select class="chosen-select" name="codigo" data-placeholder="-- Seleccionar Medicamento --" onchange="tampil_obat(this)" autocomplete="off" required>
                    <option value=""></option>
                    <?php
                    $query_obat = mysqli_query($mysqli, "SELECT RegIcaAlimento, NombreAlimento FROM registrosalimentos ORDER BY NombreAlimento ASC")
                      or die('error ' . mysqli_error($mysqli));
                    while ($data_obat = mysqli_fetch_assoc($query_obat)) {
                      echo "<option value=\"$data_obat[RegIcaAlimento]\"> $data_obat[RegIcaAlimento] | $data_obat[NombreAlimento] </option>";
                    }
                    ?>
                  </select>
                </div>
            ...
            <div class="box-footer">
              <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                  ...
                  <a href="?module=inventario_alimentos" class="btn btn-default btn-reset">Cancelar</a>
                </div>
              </div>
            </div><!-- /.box footer -->
          </form>
